...
async def test(page: Page):
    ...
    # PRIMARY: the Products page is visible | HINTS: page title near the top left of t

    # Assertion check

    # Clicking Add to cart for Sauce Labs Backpack
    _loc_1 = page.locator("[data-test=\"add-to-cart-sauce-labs-backpack\"]")

    await _loc_1.click()

    # Clicking Remove button for Sauce Labs Backpack
    _loc_2 = page.locator("[data-test=\"remove-sauce-labs-backpack\"]")

    await _loc_2.click()

    # PRIMARY: exact visible text on the action button for the Sauce Labs Backpack pro

    # Assertion check
    await pw_expect(page.get_by_text('Add to cart', exact=True)).to_be_visible()
